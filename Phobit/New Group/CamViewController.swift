...
//  Phobit
//
//  Created by Julian Kronlachner on 21.03.18.
...
//        view.bringSubview(toFront: whiteboard)
        ...
        view.bringSubview(toFront: camButton)
        ...
        if let qrCodeFrameView = qrCodeFrameView {
            qrCodeFrameView.layer.borderColor = FlatRed().cgColor
        ...
        if (device?.hasTorch)! {
            do {
                ...
                if (device?.torchMode == AVCaptureDevice.TorchMode.on) {
                    flashButton.image = #imageLiteral(resourceName: "icons8-flash-on-50")
                    device?.torchMode = AVCaptureDevice.TorchMode.off
                } else {
                    //                    do {
                    //device?.torchMode = AVCaptureDevice.TorchMode.on
                    animateTorch(time: 0.1, flashLevel: 0.1)
                    flashButton.image = #imageLiteral(resourceName: "icons8-flash-on-filled-50")
                    //                    } catch {
                    //                        print(error)
                    //                    }
                }
...
//            print("Datum: \(self.bill.datum)")
            if(/**self.bill.datum == "" || self.bill.gesamtBrutto == 0.0**/ false){
